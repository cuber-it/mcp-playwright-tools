...
:mod:`mcp_playwright_tools.tools.mcp_playwright_grant` and never by the tools.
The server reads it at every check, so a grant takes effect at the next tool
...
from mcp_playwright_tools.boundary import MODES, Boundary
from mcp_playwright_tools.errors import GrantError

GRANT_SCRIPT = Path(__file__).resolve().parents[2] / "scripts" / "grant.sh"
GRANT_MODULE = "mcp_playwright_tools.tools.mcp_playwright_grant"
GRANT_FILE = "grant.json"
DEFAULT_STATE_DIR = "~/.mcp-playwright-tools"
