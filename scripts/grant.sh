...
#
# Runs mcp_playwright_tools.tools.mcp_playwright_grant with the interpreter of
# this repository's venv, so it can be called from anywhere, also through a
# symlink:
#
...
exec "$python" -m mcp_playwright_tools.tools.mcp_playwright_grant "$@"
